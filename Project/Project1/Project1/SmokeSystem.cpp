...
namespace View {
	...
	SmokeSystem::SmokeSystem(Common &common, Vec2 scale, Vec2 screen, Vec2 startPosition) {
		InitParams(common);
		//16 size of image.
		InitEmitter(16 / 1.f, scale, screen, startPosition);
	}
	...
	void SmokeSystem::GetParams() {
		params.mMaxParticles = 140;
		params.mMinParticles = 100;
		params.mMinLifeTimeMs = 0;
		params.mMaxLifeTimeMs = 6500;
		params.mGravity = Vec2(0, 9.82);
	}

	void SmokeSystem::InitParticle(Model::Particle *p) {
		...
		p->mTint = Color::White;
		p->mScale = Vec2(1, 1);
		p->mAngularVelocity = r2() * .01f;
		p->mDir = { InRange(-.5f, .5f), InRange(-.5f, .5f) };
	...
	void View::SmokeSystem::RenderEmitter(Renderer2D *renderer) {
		// set Particle size.
		int width, height;
